...
class MenuApplication(QMainWindow):
    ...
    def init_ui(self):
        ...
        if num_columns > 1:
            ...
            main_menu_layout.setAlignment(Qt.AlignTop)  # Align to top to prevent stretching
        else:
        ...
        for i, separator in enumerate(menu_items):
            ...
            if button_info:
                # Format tooltip with HTML for larger, bolder text and white background
                formatted_info = f'<div style="background-color: white; color: black; padding: 30px; border: 1px solid #ccc; border-radius: 1px; font-size: 12pt; font-weight: bold; min-width: 500px;">{button_info.strip()}</div>'
                separator_button.setToolTip(formatted_info)
                separator_button.setToolTipDuration(15000)  # Show for 15 seconds
        ...
        if num_columns > 1:
            ...
            # Add exit button in the next row
        ...
        output_layout.setContentsMargins(5, 5, 5, 5)  # Reduce margins inside output frame
        ...
        output_title.setFont(QFont("", 12, QFont.Bold))
        output_title.setStyleSheet("color: #333333;")  
        ...
        self.output_text.setMinimumHeight(300)  # Set minimum height for output area
        ...
        # Remove any stretching in the main layout
        main_layout.setSpacing(5)  # Exact 5 pixel spacing between widgets
        
        # Add the output frame to the main layout
    ...
    def create_submenu_page(self, separator):
        ...
        spacer_after_title.setFixedHeight(5)
        ...
        for item in items:
            ...
            if item_name and command:
                ...
                # Create and apply bold font directly - match the main menu buttons
                button_font = QFont()
                button_font.setPointSize(10)  # Equivalent to about 14px
                button_font.setBold(True)
                button_font.setWeight(75)  # Explicitly set weight (50=normal, 75=bold)
                item_button.setFont(button_font)
                
                # Add tooltip if button_info is provided
                button_info = item.get('button_info', '')
                if button_info:
                    # Format tooltip with HTML for larger, bolder text
                    formatted_info = f'<span style="font-size: 12pt; font-weight: bold;">{button_info.strip()}</span>'
                    item_button.setToolTip(formatted_info)
                    item_button.setToolTipDuration(5000)  # Show for 5 seconds
                
                # Style the button - keep the same background and color styling
        ...
        # Apply bold font to back button too for consistency
        back_font = QFont()
        back_font.setPointSize(10)  # Equivalent to about 14px
        back_font.setBold(True)
        back_font.setWeight(75)
        back_button.setFont(back_font)
